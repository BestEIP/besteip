...
(function ($, undefined) {
    ...
    var checkFileList = function (files, dropped) {
        ...
        var TotalmaxSize = this.settings.TotalmaxSize || false;

        var maxFiles = this.settings.maxFiles || 20;

        if (!(allowExt || denyExt || allowMime || denyMime || maxSize || TotalmaxSize || maxFiles)) return true;//no checking required
        ...
        for (var f = 0; f < files.length; f++) {
            ...
            if (f >= maxFiles) {
                if (!('maxfiles' in error_list)) error_list['maxfiles'] = [];
                error_list['maxfiles'].push(filename);
                continue;
            }
            ...
            if (TotalmaxSize && totalSize > TotalmaxSize) {
                if (!('totalsize' in error_list)) error_list['totalsize'] = [];
                error_list['totalsize'].push(filename);
                continue;
            }
        ...
        /////////
        var error_count = { 'ext': 0, 'mime': 0, 'size': 0, 'totalsize': 0, 'maxfiles': 0 }
        ...
        if ('totalsize' in error_list) error_count['totalsize'] = error_list['totalsize'].length;
        if ('maxfiles' in error_list) error_count['maxfiles'] = error_list['maxfiles'].length;
