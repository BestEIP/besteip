...
* 20170726 修改跳出視窗關閉問題 line 15413 for(var s in this[i]) 後面增加 {}
* 20170802 12278 註記掉 inline edit fource 問題
* 20170807 15394 修改黑底位置
* 20170906 1438 tbot < dh 改為 tbot <= dh
* 20180315 15442~15477 跳出視窗位置修改(hideover相關)
    ...
    $.jgrid.extend({
        ...
        setGridWidthP: function (nwidth) {
            return this.each(function () {
                ...
                $("#gview_" + $.jgrid.jqID($t.p.id) + " .ui-jqgrid-htable").css("width", nwidth + "%");
                $($t.grid.bDiv).css("width", nwidth + "%");
                //$($t.grid.hDiv).css("width",nwidth+"%");

                if ($($t.grid.bDiv)[0].clientHeight < $($t.grid.bDiv)[0].scrollHeight) {
                    //$($t.grid.hDiv).css("width",(nwidth-1.5)+"%");
                    $($t.grid.hDiv).css("width", "calc(100% - 16px)");
                } else {
                    $($t.grid.hDiv).css("width", nwidth + "%");
                }
    ...
    */
    ...
    $.jqm = {
        ...
        open: function (s, t) {
            ...
            if (o) { h.w.before(o.addClass(c.overlayClass)) };
            h.o = (o) ? o : F;
            ...
            if ($('.jqmWindow.jqmEQ, .jqmWindow2.jqmEQ').length) {
                var parDiv = $(h.w).parents('.jqmWindow, .jqmWindow2').length ? $($(h.w).parents('.jqmWindow, .jqmWindow2')[0]) : undefined;
                $('body').addClass('hideover');
                if (parDiv !== undefined) {
                    parDiv.addClass('hideover');
                    if ((parDiv.attr('id').indexOf('dialog_ei') >= 0 || parDiv.attr('id').indexOf('dialog_PopUpOuter') >= 0))
                        $('body').removeClass('hideover');
                }
                $(h.w).scrollTop(0);
                $('[id^="dialog_ei"], [id^="dialog_PopUpOuter"]').removeClass('hideover');
            }
            return F;
        },
        close: function (s) {
            ...
            if ($('.jqmWindow.jqmEQ, .jqmWindow2.jqmEQ').length) {
                $($(h.w).parents('.jqmWindow, .jqmWindow2')[0]).removeClass('hideover');
                var bol = true;
                $('.jqmWindow.jqmEQ, .jqmWindow2.jqmEQ').each(function (a, b) {
                    bol = bol ? !($(b).css('display') == 'block') : bol;
                });
                if (bol)
                    $('body').removeClass('hideover');
            }
